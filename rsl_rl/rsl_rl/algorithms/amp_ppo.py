...
from rsl_rl.modules import Discriminator, ActorCritic
...
class AMPPPO:
    ...
    def __init__(
        ...
        self.optimizer = optim.Adam(params, lr=discriminator_learning_rate)
    ...
    def process_env_step(self, obs, rewards, dones, extras, next_critic_obs, amp_observation_buf):
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_expert_data in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            obs_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, hid_states_batch, masks_batch = sample
            ...
            if self.symmetry and self.symmetry["use_data_augmentation"]:
                ...
                # obs_batch["next_critic_obs"] = next_critic_obs_batch
            ...
            # estimation_loss, swap_loss = self.policy.him_estimator.update(
            #     obs_batch[:original_batch_size], next_critic_obs_batch, lr=self.learning_rate
            # )
            ...
            if self.amp_state_normalizer is not None:
                for i in range(self.discriminator.observation_horizon):
                    with torch.no_grad():
                        policy_state_buf[:, i] = self.amp_state_normalizer.normalize_torch(
                            sample_amp_policy[:, i], self.device
                        )
                        expert_state_buf[:, i] = self.amp_state_normalizer.normalize_torch(
                            sample_amp_expert[:, i], self.device
                        )
