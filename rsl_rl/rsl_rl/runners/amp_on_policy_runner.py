        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    next_privileged_obs = obs["critic"].clone().detach()
                    # Move to device
                    obs, rewards, dones = (obs.to(self.device), rewards.to(self.device), dones.to(self.device))
                    # process termination obs
                    termination_ids = extras["terminal_observations"].get("env_ids")
                    termination_privileged_obs = extras["terminal_observations"].get("critic")
                    ...
                    self.alg.process_env_step(
                        obs, rewards, dones, extras, next_privileged_obs, amp_observation_buf_with_term
                    )
