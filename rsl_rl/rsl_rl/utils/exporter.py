...
class _OnnxPolicyExporter(torch.nn.Module):
    ...
    def __init__(self, policy, normalizer=None, verbose=False):
        ...
        if hasattr(policy, "actor"):
            ...
            #     self.him_estimator = copy.deepcopy(policy.him_estimator.encoder)
    ...
    def forward_him(self, obs):
        ...
        parts = self.him_estimator(x_in)
        vel, z = parts[..., :3], parts[..., 3:]
        z = F.normalize(z, dim=-1, p=2.0)
        return self.actor(torch.cat((x_in[:, -self.num_one_step_obs:], vel, z), dim=-1))
    ...
    def export(self, path, filename):
        ...
        if self.is_recurrent:
            ...
            if hasattr(self, "him_estimator"):
                obs = torch.zeros(1, self.him_estimator[0].in_features)
                torch.onnx.export(
                    self,
                    obs,
                    os.path.join(path, filename),
